...
router.get('/uploads/:page', async (req, res) => {
  ...
  const uploads = await uploadsLisingPerPage(
    req,
    res,
    req.params.page,
    10,
...
router.get('/uploads/delete/all', async (req, res) => {
  const uploads = await Upload.find({}); // If no uploads are found then show a error message
  ...
  req.flash('success', `Removed all uploads`);
  res.redirect('back');
});
...
router.patch('/users/:id/suspend', async (req, res) => {
  ...
  req.flash(
    'success',
    `${account.username} has been suspend till ${moment(expire).format(
      'M/D/YYYY h:mm A'
    )} UTC`
  );
...
router.patch('/users/:id/unsuspend', async (req, res) => {
  ...
  account.save();
  req.flash('success', `${account.username} has been unsuspend.`);
  res.redirect('/admin/users');
});
...
router.post('/users/new', (req, res) => {
  ...
  // Password
  if (!password) {
    error.password = 'Must have a pssword';
  }
