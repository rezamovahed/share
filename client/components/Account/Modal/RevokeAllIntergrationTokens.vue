<template>
  ...
  >
    ...
    <div
      ...
      @click.stop
    >
      <form @submit.prevent="revokeIntergrationTokens">
            ...
            <div class="my-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <div>
                <h3 class="text-xl font-medium leading-6 text-red-500">
                  Are you sure you want to revoke all intergration tokens?
                </h3>
              </div>
              <div class="my-3">
                <p
                ...
                >
                  This will revoke all your intergrations. Can't be undone and
                  you will have to relogin after you click comfirm.
                </p>
              </div>
            </div>
          </div>
          <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            >
              <button
                ...
                @click="hideRevokeAllIntegrationTokensModal"
                @keydown.esc="hideRevokeAllIntegrationTokensModal"
              >
                Cancel
              </button>
            </span>
...
export default {
  mounted() {
    ...
    const close = (e) => {
      ...
      this.hideRevokeAllIntegrationTokensModal()
    }
  ...
  methods: {
    ...
    async hideRevokeAllIntegrationTokensModal() {
      await this.$store.commit(
        'account/SET_SHOW_REVOKE_ALL_INTERGRATION_TOKENS_MODAL',
        false
      )
      await this.$store.commit('account/SET_MESSAGE_SUCCESS', null)
      await this.$store.commit('account/SET_MESSAGE_ERROR', null)
    },
    async revokeIntergrationTokens() {
      try {
        await this.$store.dispatch('account/REVOKE_INTERGATION_TOKENS')
